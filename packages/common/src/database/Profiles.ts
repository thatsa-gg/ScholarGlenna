...
export class Profiles {
    ...
    async import(source: DiscordUser): Promise<Profile> {
        return await this.#database.Client.$transaction<Profile>(async client => {
            ...
            await this.importMemberships(source, client)
            return result
        })
    }

    private async importMemberships(source: DiscordUser, client: Prisma.TransactionClient){
        ...
        await client.$executeRaw`call import_profile_guilds(${source.id}::snowflake, new_snowflake())`
